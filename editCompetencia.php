<?php
	...
	if (isset($_POST['submit'])) {		
        $sql = $conn->prepare("UPDATE Competencia SET nome=?  WHERE id=?");
		$nome = $_POST['nome'];
		$sql->bind_param("si", $nome, $_GET["id"]);	
		if($sql->execute()) {
			$success_message = "Alterado com sucesso";
		} else {
			$error_message = "Erro ao alterar";
		}

	}
	$sql = $conn->prepare("SELECT * FROM Competencia WHERE id=?");
...
<div class="button_link"><a href="addCompetencia.php" > Voltar </a></div>

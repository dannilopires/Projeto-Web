<?php
	if (isset($_POST['submit'])) {
		require_once("db.php");
		$sql = $conn->prepare("INSERT INTO Competencia (nome) VALUES (?)");  
		$nome = $_POST['nome'];
...
<table cellpadding="10" cellspacing="0" width="500" class="tbl-qa">
	<thead>
		<tr>
			<th colspan="2" class="table-header">Adicionar nova competência</th>
		</tr>
	</thead>
	<tbody>
		<tr class="table-row">
...
<table class="tbl-qa">	
		<thead>
			 <tr>
				<th class="table-header" width="40%"> Nome da Competência </th>
		...
		<tbody>		
			...
			<tr class="table-row" id="row-<?php echo $row["id"]; ?>"> 
                ...
                <!--ação-->        
				<td class="table-row" colspan="2" text-align="right"><a href="editCompetencia.php?id=<?php echo $row["id"]; ?>" class="link"><img title="Edit" src="icon/edit.png"/></a> <a href="deleteCompetencia.php?id=<?php echo $row["id"]; ?>" class="link"><img name="delete" id="delete" title="Delete" onclick="return confirm('Tem certeza de que deseja deletar?')" src="icon/delete.png"/></a></td>
			</tr>
			<?php
                    }
				}
